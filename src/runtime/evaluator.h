...
public:
  ...
  const std::optional<RuntimeError>& last_error() const {
    return err_;
  }
...
private:
  ...
  // Output destination used by print statements.
  // runner.cpp sets this to stdout_stream so browser output can be captured.
  ...
  bool exec_func_decl(const FuncDecl* f);
