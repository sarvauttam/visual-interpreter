...
async function loadInterpreterModule() {
  const factory =
    ...
    (typeof window !== "undefined" &&
      typeof window.VisualInterpreterModule === "function" &&
      window.VisualInterpreterModule) ||
    (typeof Module === "function" && Module) ||
    (typeof window !== "undefined" &&
      typeof window.Module === "function" &&
      window.Module);
...
(function () {
  ...
  function highlightMatchingExplanationCard() {
    ...
    const info = getCursorLineInfo();
    const cards = explanationsPanel.querySelectorAll(
      ".explanation-block[data-line-number]"
    );
  ...
  function applyEditorStyles() {
    ...
    sourceInput.style.textDecoration = editorStyleState.underline
      ? "underline"
      : "none";
  }
  ...
  function saveHistoryItem(item) {
    try {
      ...
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(current.slice(0, MAX_HISTORY_ITEMS))
      );
  ...
  function classifyLineState(line) {
    ...
    if (!trimmed) {
      return { kind: "empty", complete: false, explainable: false };
    }

    if (/^#include\s*</.test(trimmed) && !/>$/.test(trimmed)) {
      return { kind: "incomplete_include", complete: false, explainable: true };
    }
    if (/^#include\s*<[^>]+>\s*$/.test(trimmed)) {
      return { kind: "include", complete: true, explainable: true };
    }
    ...
    if (/^using\s+namespace\s+\w+\s*;\s*$/.test(trimmed)) {
      return { kind: "namespace", complete: true, explainable: true };
    }
    ...
    if (/^int\s+main\s*\(\)\s*\{$/.test(trimmed)) {
      return { kind: "main_start", complete: true, explainable: true };
    }

    if (/^if\s*\(.*$/.test(trimmed) && !/\)\s*\{?\s*$/.test(trimmed)) {
      return { kind: "incomplete_if", complete: false, explainable: true };
    }
    if (/^if\s*\([^)]+\)\s*$/.test(trimmed)) {
      return { kind: "if_waiting_block", complete: false, explainable: true };
    }
    if (/^if\s*\([^)]+\)\s*\{$/.test(trimmed)) {
      return { kind: "if_start", complete: true, explainable: true };
    }

    if (/^else\s*$/.test(trimmed)) {
      return { kind: "else_waiting_block", complete: false, explainable: true };
    }
    if (/^else\s*\{$/.test(trimmed)) {
      return { kind: "else_start", complete: true, explainable: true };
    }

    if (/^while\s*\(.*$/.test(trimmed) && !/\)\s*\{?\s*$/.test(trimmed)) {
      return { kind: "incomplete_while", complete: false, explainable: true };
    }
    ...
    if (/^while\s*\([^)]+\)\s*\{$/.test(trimmed)) {
      return { kind: "while_start", complete: true, explainable: true };
    }

    if (/^for\s*\(.*$/.test(trimmed) && !/\)\s*\{?\s*$/.test(trimmed)) {
      return { kind: "incomplete_for", complete: false, explainable: true };
    }
    if (/^for\s*\([^)]+\)\s*$/.test(trimmed)) {
      return { kind: "for_waiting_block", complete: false, explainable: true };
    }
    if (/^for\s*\([^)]+\)\s*\{$/.test(trimmed)) {
      return { kind: "for_start", complete: true, explainable: true };
    }

    if (/^return\s*$/.test(trimmed)) {
      return { kind: "incomplete_return", complete: false, explainable: true };
    }
    if (/^return\b/.test(trimmed) && !/;\s*$/.test(trimmed)) {
      return { kind: "unfinished_return", complete: false, explainable: true };
    }
    if (/^return\b.*;\s*$/.test(trimmed)) {
      return { kind: "return", complete: true, explainable: true };
    }

    if (/^cout\s*<<\s*$/.test(trimmed) || /^print\s*\(\s*$/.test(trimmed)) {
      return { kind: "incomplete_output", complete: false, explainable: true };
    }
    if (
      (/^cout\s*<</.test(trimmed) || /^print\s*\(/.test(trimmed)) &&
      !/;\s*$/.test(trimmed)
    ) {
      return { kind: "unfinished_output", complete: false, explainable: true };
    }
    if (
      (/^cout\s*<</.test(trimmed) || /^print\s*\(/.test(trimmed)) &&
      /;\s*$/.test(trimmed)
    ) {
      return { kind: "output", complete: true, explainable: true };
    }

    if (/^cin\s*>>\s*$/.test(trimmed) || /^input\s*\(\s*$/.test(trimmed)) {
      return { kind: "incomplete_input", complete: false, explainable: true };
    }
    if (
      (/^cin\s*>>/.test(trimmed) || /^input\s*\(/.test(trimmed)) &&
      !/;\s*$/.test(trimmed)
    ) {
      return { kind: "unfinished_input", complete: false, explainable: true };
    }
    if (
      (/^cin\s*>>/.test(trimmed) || /^input\s*\(/.test(trimmed)) &&
      /;\s*$/.test(trimmed)
    ) {
      return { kind: "input", complete: true, explainable: true };
    }

    if (/^\{$/.test(trimmed)) {
      return { kind: "open_brace", complete: true, explainable: true };
    }
    if (/^\}$/.test(trimmed)) {
      return { kind: "close_brace", complete: true, explainable: true };
    }
    ...
    if (/^let\s+\w+\s*=.*;\s*$/.test(trimmed)) {
      return { kind: "declaration", complete: true, explainable: true };
    }
    ...
    if (/^\w+\s*=.*;\s*$/.test(trimmed) && !/==/.test(trimmed)) {
      return { kind: "assignment", complete: true, explainable: true };
    }
    ...
    if (/^func\s+\w+\s*\([^)]*\)\s*\{$/.test(trimmed)) {
      return { kind: "function_header", complete: true, explainable: true };
    }

    if (trimmed.endsWith(";")) {
      return { kind: "generic_statement", complete: true, explainable: true };
    }
    if (trimmed.endsWith("{")) {
      return { kind: "generic_block_start", complete: true, explainable: true };
    }

    return { kind: "unfinished_unknown", complete: false, explainable: true };
  }
        ...
        ? `
          <div class="explanation-block explanation-block--pending explanation-block--active" data-line-number="${lastNonEmpty.lineNumber}">
            ...
            <p class="explanation-text explanation-text--pending">
              ${escapeHtml(
                getLineHint(lastNonEmpty.line) ||
                  "Keep going — this line looks incomplete, so I am waiting before explaining it fully."
              )}
            </p>
          </div>
        `
  ...
  function showHowToUse() {
    window.alert(
      [
        ...
        "2. Watch the explanation panel react while you type.",
